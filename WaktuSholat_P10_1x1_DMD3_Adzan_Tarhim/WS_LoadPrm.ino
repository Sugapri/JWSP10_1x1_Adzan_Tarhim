 ...
 *      4   byte add  1   float         Latitude         code NLA // Koordinat SBY -7.271748, 112.736151
 *      4   byte add  5   float         Longitude        code NLO
 *      4   byte add  9   float         Altitude         code NAL   //NAL Harus 0
 *      4   byte add 13   float         TimeZone         code NTZ
 *      1   byte add 17   uint8_t       Masjid Type      code NMT
 *      1   byte add 18   uint8_t       Led Brighnest    code NBL //Kecerahan
        ...
        while ((bchar != '\n')and(prm_idx < 149))
          ...
          if(Serial.available())
            { 
              ...
              prm_idx ++;
              digitalWrite(BUZZ, HIGH);////////BEEP RECEIVE COMMAND
              delay(10);
...
void set_default_prm()
      {
        ...
        Prm = (struct_param){212,-6.39524,106.80256,45,7,1,200,2,10,30,15,10,10,7,10,1,1,1,1,0};
        EEPROM.put(0,Prm);
//        EEPROM.put(55, "Y2K_TECHNOLOGI\0");
        EEPROM.put(130, "Info 1\0");
//        EEPROM.put(280,"Info 2\0");
//        EEPROM.put(430,"Info 3\0");
        EEPROM.put(580,"SAATNYA SHALAT\0");
        EEPROM.put(730,"SHALAT JUM'AT\0");
